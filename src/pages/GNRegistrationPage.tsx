...
const GNRegistration: React.FC = () => {
  ...
  // Updated signature state to handle both dataUrl and File
  ...
  const validateForm = () => {
    ...
    if (!signatureDataUrl || !signatureFile) newErrors.signature = 'Digital signature is required';

    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };

  // Updated signature handler to match ApplicationViewer pattern
  const handleSignature = async (dataUrl: string, file?: File): Promise<void> => {
  ...
  try {
    // Check if your API service has a method that accepts FormData with signature
    if (typeof userApiService.createGNWithSignature === 'function') {
      // OPTION 1: Use FormData for file upload (with signature)
      ...
      registrationFormData.append('role', UserRole.GN);
      registrationFormData.append('divisionId', formData.gnDivisionId);
      
      // Add additional data as JSON string
      const additionalData = {
        nic: formData.nic.trim()
      };
      registrationFormData.append('additionalData', JSON.stringify(additionalData));
      
      // Add signature file
      if (signatureFile) {
        registrationFormData.append('signature', signatureFile);
      } else if (signatureDataUrl) {
        // Convert dataURL to File
        const response = await fetch(signatureDataUrl);
        const blob = await response.blob();
        const file = new File([blob], "signature.png", { type: blob.type || "image/png" });
        registrationFormData.append('signature', file);
      }

      // Call the API service with FormData
      const createdUser = await userApiService.createGNWithSignature(registrationFormData);
      
    } else {
      // OPTION 2: Use regular JSON API call (original working method)
  ...
  additionalData: {
    nic: formData.nic.trim(),
    signatureDataUrl: signatureDataUrl  // Add signature here
  },
  phone: formData.phone.trim(),
  role: UserRole.GN,
  divisionId: formData.gnDivisionId,
  password: formData.password,
};

      // Call the original API service
