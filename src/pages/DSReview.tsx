...
const DSReview: React.FC = () => {
  ...
  useEffect(() => {
    const loadApplicationData = async () => {
      ...
      try {
        ...
        const appResponse = await applicationApiService.getApplicationById(
          applicationId
        );
        console.log("Application loaded:", appResponse);
        setApplication(appResponse);

        const certifyingSignature = appResponse.attachments.find(
          (doc) => doc.attachmentType === "CERTIFY_SIGNATURE"
        );

        if (certifyingSignature?.uploadedByUser) {
          const gnUser = await userApiService.getUserById(
            certifyingSignature.uploadedByUser.id
          );
        ...
        if (appResponse.currentStatus !== ApplicationStatus.APPROVED_BY_GN) {
          setError(
            `This application is not ready for DS review. Current status: ${appResponse.currentStatus}`
          );
        }
  ...
  const loadDocumentsAndLogs = async (appId: string) => {
    ...
    try {
      ...
      const attachments: Attachment[] = Array.isArray(docsResponse)
        ? docsResponse.map((doc: any) => ({
            id: doc.id,
            attachmentType: doc.attachmentType || "APPLICATION_ATTACHMENT",
            fileName:
              doc.fileName ||
              doc.originalFileUrl?.split("/").pop() ||
              "Unknown",
            ...
            createdAt:
              doc.createdAt || doc.uploadedAt || new Date().toISOString(),
  ...
  const handleSendToDRP = async () => {
    ...
    try {
      const updatedApp = await applicationApiService.updateApplicationStatus(
        application.id,
        {
          status: ApplicationStatus.SENT_TO_DRP,
          comment:
            "Application approved by DS and sent to DRP for final processing",
        }
      );
      ...
      toast.success(
        "Application successfully sent to DRP for final processing"
      );
  ...
  const getGNApplicationSignature = () => {
    return application.attachments.find(
      (doc) => doc.attachmentType === "CERTIFY_SIGNATURE"
    );
  };
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              DS Review for Application #
              {application.id.slice(-8).toUpperCase()} • Submitted{" "}
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Badge variant="outline">
                    {application.user.division?.name || "Unknown Division"}
                  </Badge>
                </div>
                {application?.user?.additionalData?.nic && (
                  <div>
                    <Label className="text-sm font-medium text-muted-foreground">
                      GN NIC
                    </Label>
              ...
              {application.applicationType === "new_nic" && (
                <Alert>
                  ...
                  <AlertDescription>
                    <strong>New NIC Application:</strong> This applicant does
                    not have an existing NIC number. After DS approval, send to
                    DRP for NIC generation.
                  </AlertDescription>
                </Alert>
              )}
            </CardContent>
          ...
          {application.applicationData && (
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {application.applicationData.name && (
                    <div>
                      ...
                      <p className="font-medium">
                        {application.applicationData.name}{" "}
                        {application.applicationData.surname}
                      </p>
                    </div>
                  )}
                  {application.applicationData.dateOfBirth && (
                    <div>
                      ...
                      <p className="flex items-center space-x-2">
                        ...
                        <span>
                          {formatDate(application.applicationData.dateOfBirth)}
                        </span>
                      </p>
                    </div>
                  )}
                ...
                {/* Address Information */}
                {(application.applicationData.street ||
                  application.applicationData.city) && (
                  <div className="pt-4 border-t">
                    <Label className="text-sm font-medium text-muted-foreground">
                      Address
                    </Label>
                    <div className="mt-2 text-sm">
                      {application.applicationData.houseNumber && (
                        <span>{application.applicationData.houseNumber}, </span>
                      )}
                      {application.applicationData.street && (
                        <span>{application.applicationData.street}, </span>
                      )}
                      {application.applicationData.city && (
                        <span>{application.applicationData.city}</span>
                      )}
                      {application.applicationData.postalCode && (
                        <span> - {application.applicationData.postalCode}</span>
                      )}
                    </div>
                  </div>
                )}
                              ...
                              <div>
                                ...
                                {doc.uploadedByUser && (
                                  <p className="text-xs text-muted-foreground">
                                    By: {doc.uploadedByUser.firstName}{" "}
                                    {doc.uploadedByUser.lastName}
                                  </p>
                                )}
                              </div>
                            </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <Alert>
                  ...
                  <AlertDescription>
                    This application has been verified by the GN and is ready
                    for DS approval. Once approved, it will be sent to the
                    District Registrar of Properties (DRP).
                  </AlertDescription>
                </Alert>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Compare the GN's signature on the application with their
            ...
            <CardContent className="space-y-4">
                    ...
                    {gnApplicationSignature ? (
                      <div className="text-center w-full">
                        <img
                          ...
                          onError={(e) => {
                            console.log(
                              "Error loading application signature image",
                              e
                            );
                            const target = e.target as HTMLImageElement;
                            target.style.display = "none";
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-sm text-red-600">
                          No application signature found
                        </p>
                      </div>
                    )}
                  </div>
              ...
              gnApplicationSignature ? (
                <Alert>
                  <CheckCircle className="h-4 w-4" />
                  <AlertDescription>
                    <strong>Signature Status:</strong> Both registered and
                    application signatures are available for verification.
                ...
                <Alert>
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>
                    <strong>Signature Warning:</strong>{" "}
                    {!gnUser?.additionalData?.signatureUrl &&
                      "Registered signature is missing. "}
                    {!gnApplicationSignature &&
                      "Application signature is missing. "}
                  ...
                  <div key={log.id || index} className="flex space-x-3">
                    ...
                    <div className="flex-1 pb-4">
                      <div className="flex items-center space-x-2 mb-1">
                        <StatusBadge
                          status={log.status || log.toStatus}
                          size="sm"
                        />
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              applicant and GN will be notified.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
      ...
      <Dialog open={showHoldDialog} onOpenChange={setShowHoldDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Please provide a reason for putting this application on hold. The
              applicant and GN will be notified.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
